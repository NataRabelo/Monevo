...
def acessarTransacao():
    if request.method == "GET":
        # filtrar por usuário para garantir que categorias/contas/transacoes visíveis pertençam ao usuário
        cartoes = Cartoes.query.filter_by(usuario_id=current_user.id).all()
        contas = Contas.query.filter_by(usuario_id=current_user.id).all()
        transacoes = Transacoes.query.filter_by(usuario_id=current_user.id).all()
        categorias = Categorias.query.filter_by(usuario_id=current_user.id).all()
...
def editarTransacao():
    try:
        ...
        if tipo == 'Receita':
            ...
            # Atualizando campos (nomes alinhados com o HTML)
...
def deletarTransacao(transacao_id):
    try:
        # VALIDA A EXISTENCIA 
        transacao = Transacoes.query.filter(Transacoes.id == transacao_id, Transacoes.usuario_id == current_user.id).first()
